...
func (d *DB) Sync() error {
	...
	var lastId string
	err := d.s.Db.QueryRow("MIO_GET_LAST_TX", sql.Args{"group": d.groupName, "storeUrl": d.s.Store.Url()}, &lastId)
	...
	for _, l := range ls {
		if l.Name() > lastId {
			...
			_, err = d.s.Db.Exec("MIO_STORE_TX", sql.Args{"group": tx.GroupName, "storeUrl": d.s.Store.Url(), "id": l.Name(), "version": 0, "consumed": false})
			if err != nil {
				return err
			}
			...
			_, err = d.s.Db.Exec("MIO_STORE_TX", sql.Args{"group": tx.GroupName, "storeUrl": d.s.Store.Url(), "id": l.Name(), "version": tx.Version, "consumed": true})
			if err != nil {
				sqlTx.Rollback()
				continue
			}

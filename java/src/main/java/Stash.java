...
public class Stash {
    ...
    static public Stash create(DB db, Identity identity, String url, 
                Config config ) throws Exception {
        Result r = StashLibrary.instance.stash_createStash(db.hnd, identity.toJson(), url, config.toJson());
        r.check();
        r.check();
        return new Stash(r.hnd);
    }

    static public Stash open(DB db, Identity identity, String url) throws Exception {
        Result r = StashLibrary.instance.stash_openStash(db.hnd, identity.toJson(), url);
        r.check();
        return new Stash(r.hnd);
    }

    public void close()  {
        StashLibrary.instance.stash_closeStash(hnd);
    }

    private Stash(long hnd) {
        this.hnd = hnd;
    }

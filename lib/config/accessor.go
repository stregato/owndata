...
func GetConfigValue(db *sqlx.DB, domain string, key string) (s string, i int64, v []byte, ok bool) {
	err := db.QueryRow("GET_CONFIG", sqlx.Args{"node": domain, "key": key}, &s, &i, &v)
	switch err {
	case sqlx.ErrNoRows:
		ok = false
	case nil:
	...
	core.Trace("SQL: GET_CONFIG: %s/%s - ok=%t, %s, %d, %v", domain, key, ok, s, i, v)
	return s, i, v, ok
}

func SetConfigValue(db *sqlx.DB, domain string, key string, s string, i int64, v []byte) error {
	_, err := db.Exec("SET_CONFIG", sqlx.Args{"node": domain, "key": key, "s": s, "i": i, "b": v})
	if err != nil {
		return core.Errorw(err, "cannot set config %s/%s with values %s, %d, %v: %v", domain, key, s, i, v)
	}
	core.Trace("SQL: SET_CONFIG: %s/%s - %s, %d, %v", domain, key, s, i, v)
	return nil
}

func ListConfigKeys(db *sqlx.DB, domain string) ([]string, error) {
	rows, err := db.Query("LIST_CONFIG", sqlx.Args{"node": domain})
...
func DelConfigNode(db *sqlx.DB, domain string) error {
	_, err := db.Exec("DEL_CONFIG", sqlx.Args{"node": domain})
	if err != nil {

...
class Safe():
    ...
    def messanger(self):
        rq = lib.stash_openMessanger(self.hnd)
        consume(rq)
...
class Database:
    ...
    def get_counter(self, table: str, key: str):
        r = lib.stash_get_counter(self.hnd, e8(table), e8(key))
        return consume(r)
    
    def transaction(self):
        r = lib.stash_transaction(self.hnd)
        consume(r)
        transaction = Transaction()
        transaction.hnd = r.hnd
        return transaction
    ...
    def __del__(self):
        if self.hnd:
            lib.stash_closeDatabase(self.hnd)
...
class Transaction:
    ...
    def inc_counter(self, table: str, key: str, value: int):
        r = lib.stash_incCounter(self.hnd, e8(table), e8(key), value)
        return consume(r)
    
    def commit(self):
        r = lib.stash_commit(self.hnd)
        return consume(r)

    def rollback(self):
        r = lib.stash_rollback(self.hnd)
        return consume(r)

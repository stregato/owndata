...
func NewIdentity(nick string) (Identity, error) {
	...
	if strings.Contains(nick, ":") {
		return identity, core.Errorf("invalid nickname '%s': %w", nick, ErrInvalidID)
	}
...
func SetIdentity(i Identity) error {
	...
	_, err = sql.Default.Exec("SET_IDENTITY", sql.Args{
		"id":   i.Id,
		"data": data,
	})
	return err
...
func DelIdentity(id string) error {
	_, err := sql.Default.Exec("DEL_IDENTITY", sql.Args{
		"id": id,
	})
	return err
}
...
func GetIdentity(id string) (Identity, error) {
	var data []byte
	var identity Identity
	err := sql.Default.QueryRow("GET_IDENTITY", sql.Args{"id": id}, &data)
	if err == nil {
...
func GetIdentities() ([]Identity, error) {
	rows, err := sql.Default.Query("GET_IDENTITIES", sql.Args{})
